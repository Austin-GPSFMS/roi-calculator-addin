...
geotab.addin.roiCalculator = function (api, state) {
    ...
    const updateCalculationsUI = () => {
        ...
        // Filter Devices Globally based on group (Recursive)
        let filteredDevices = cachedData.devices;
        if (selectedGroupId && selectedGroupId !== 'all') {
            const descendants = cachedData.descendantGroupsMap[selectedGroupId] || [selectedGroupId];
            filteredDevices = cachedData.devices.filter(d => {
                const deviceGroupIds = cachedData.deviceGroupsMap[d.id + '_ids'] || [];
                // Check if device is in the selected group OR any of its descendants
                return deviceGroupIds.some(gid => descendants.includes(gid));
            });
        }

        console.log(`ROI Calculator: Filtered devices count for group ${selectedGroupId}: ${filteredDevices.length}`);
    ...
    const fetchData = async () => {
        ...
        try {
            ...
            // Fetch Fleet Info once (consolidated point of truth)
            const fleetInfo = await apiService.getFleetInfo();
            devices = fleetInfo.devices;
            const rawGroups = fleetInfo.rawGroups;
            const deviceGroupsMap = fleetInfo.deviceGroupsMap;

            if (devices.length === 0) {
                console.warn("ROI Calculator: No active devices found.");
            }

            // Build Descendant Map for recursive filtering
            const descendantGroupsMap = {};
            const childGroupsMap = {}; // ID -> [child IDs]

            rawGroups.forEach(g => {
                if (g.parent && g.parent.id) {
                    if (!childGroupsMap[g.parent.id]) childGroupsMap[g.parent.id] = [];
                    childGroupsMap[g.parent.id].push(g.id);
                }
            });
            ...
            const getDescendants = (groupId) => {
                let results = [groupId];
                const children = childGroupsMap[groupId] || [];
                children.forEach(cid => {
                    results = results.concat(getDescendants(cid));
                });
                return results;
            };

            rawGroups.forEach(g => {
                descendantGroupsMap[g.id] = getDescendants(g.id);
            });

            // Fetch metrics in sequence, each independently resilient
            try {
            ...
            console.error("ROI Calculator: Fatal load error", criticalError);
        } finally {
            // Hide loading state
